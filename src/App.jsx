...
const reducer = (state, action) => {
  switch (action.type) {
    ...
    case "remove":
      return{...state,cartCount:0,totalCost:0,prodPrice:0}
      default:
      return state;
  }
};
...
function CartPage() {
  ...
  return (
    ...
    <input className="text-danger num"
        type="number"
        onChange={(e) =>
          dispatch({ type: "change_cartCount", payload: e.target.value })
        }
        value={state.cartCount}
      />
    
      <span className="text "> 
      <p className="mt-1 ml-0 "> $.{state.prodPrice}</p>
      </span>
      ...
      <button className="text-danger m-1" onClick={() => dispatch({type:"remove"})}>
       Remove
      </button>
      ...
      <div className="margin ">
      <p className="h6 text">$.{state.totalCost}</p>
      <p className="small">FREE</p>
      </div>

      <div className="margin ">
      <p className="h6 text ml-4">$.{state.totalCost}</p>
